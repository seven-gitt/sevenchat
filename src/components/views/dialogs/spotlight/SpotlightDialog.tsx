                ...
                try {
                    if (event.getType() === "m.room.message") {
                        ...
                        if (content && content.body && typeof content.body === 'string' && 
                            ...
                            const urlsInMessage = content.body.match(URL_REGEX) || [];
                            const urlsInMessageLower = urlsInMessage.map(url => url.toLowerCase());
                            ...
                            // Check if query matches any URL in the message
                            const hasUrlMatch = urlsInMessageLower.some(url => 
                                url.includes(lcQuery) || lcQuery.includes(url.replace(/^https?:\/\//, ''))
                            );
                            
                            // Check if query looks like a URL pattern
                            const isQueryUrlLike = lcQuery.includes('.') || lcQuery.includes('://') || lcQuery.includes('/');
                            ...
                            if (hasUrlMatch) {
                                relevanceScore += 150; // High score for URL matches
                                console.log(`URL match found: query "${query}" matches URL in message`);
                            }
                            
                            // If query looks like a URL, boost URL-containing messages
                            if (isQueryUrlLike && urlsInMessage.length > 0) {
                                relevanceScore += 50;
                            }
                            ...
                            if (messageLength > 200 && !hasExactMatch && !hasUrlMatch) {
                                relevanceScore -= 10;
                            }
                            ...
                            if (matchedWords.length === 1 && matchedWords[0].length <= 2 && !hasUrlMatch) {
                                relevanceScore -= 50; // Heavy penalty for single character matches
                            }
                            
                            // Remove penalty for URLs - instead, boost them if they match
